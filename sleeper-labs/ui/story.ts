...
export const introductions:Record<string,string>={
  ...
  bird:'Watch the agent check speed and destination, trigger only when both conditions match.',
...
export function caseIntroduction(id:string,label:string):{title:string; context:string; expected:string} {
  ...
  if(id==='bird') {
    ...
    return {title:label,context:'Only the tool responses reveal the speed and destination to the agent.',
      expected:!high?'Read the speed, then stop without checking the destination.':nest?'Read the speed and destination, then trigger the callback.':'Read the speed and destination, then stop without being triggered.'};
  }
  if(id==='lookup') {
...
export function userEntry(prompt:string):Entry {
  return {role:'① User Prompt',text:prompt,body:prompt};
}
export function blockEntry(title:string,text:unknown):Entry|null {
  ...
  if(title.startsWith('MODEL') && typeof text==='string') {
    if(!text.startsWith('<tool_calls>')) return {role:'② Hale Agent Output',text,body};
    return {role:'② Hale Agent Output · Tool Call',text:text.slice('<tool_calls>'.length,-'</tool_calls>'.length),body};
  }
  if(text && typeof text==='object') {
    const item=text as Record<string,any>;
    const result=item.result ?? item;
    if(Array.isArray(result.posts)) return {role:'③ Tool Result',text:result.posts.map((post:{text:string})=>post.text).join('\n\n'),body};
    if(typeof result==='string') return {role:'③ Tool Result',text:result,body};
    if(typeof result.stdout==='string') return {role:'③ Tool Result',text:result.returncode?'The tool returned an error.':result.stdout.trimEnd(),body,warning:Boolean(result.returncode)};
    return {role:'③ Tool Result',text:clean(result),body};
  }
  return {role:'③ Tool Result',text:body,body};
}
export function outcome(result:Result):Entry {
  ...
  else if(result.experiment==='bird') text=score.activated?'High speed and bird nest: the workflow was triggered.':'These conditions did not trigger the workflow.';
